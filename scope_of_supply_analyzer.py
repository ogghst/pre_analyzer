...
class ProjectStructureAnalyzer:
    ...
    def process_file(
        self, uploaded_file, file_type: FileType = None
    ...
    def render_unified_file_upload(self, title: str):
        ...
        uploaded_file = st.sidebar.file_uploader(
            "Choose quotation file", type=file_extensions, key="unified_file_uploader", help=help_text
        )

        if uploaded_file is not None:
            # Create a unique key for this file based on name and size
            file_key = f"unified_{uploaded_file.name}_{uploaded_file.size}"

            # Check if we already have this analyzer cached in session state
            if f"unified_analyzer_{file_key}" in st.session_state:
                cached_data = st.session_state[f"unified_analyzer_{file_key}"]
                success_msg = (
                    f"✅ {uploaded_file.name} loaded from cache (Detected: {cached_data['file_type_display']})"
                )
                st.sidebar.success(success_msg)
                return cached_data["analyzer"], uploaded_file.name, cached_data["detected_type"]
            ...
            with st.spinner(f"Processing {uploaded_file.name}..."):
                try:
                    ...
                    try:
                        ...
                        # Cache the analyzer and related data in session state
                        cache_entry = {
                            "analyzer": analyzer,
                            "detected_type": detected_type,
                            "file_type_display": file_type_display,
                        }
                        st.session_state[f"unified_analyzer_{file_key}"] = cache_entry
    ...
    def render_cross_comparison_upload(self):
        ...
        uploaded_pre_file = st.sidebar.file_uploader(
            "Choose PRE file", type=file_extensions, key="cross_pre_file_uploader", help="Upload a PRE quotation file"
        )
    ...
    def create_cross_comparator(self, pre_data, prof_data, pre_name: str, prof_name: str):
        ...
        prof_hash = hash(
            str(prof_data.get("project", {}).get("id", "")) + str(len(prof_data.get("product_groups", [])))
        )
    ...
    def render_analysis_view(self, analyzer, view_name: str):
        ...
        try:
            ...
            render_error_message(
                f"Error displaying {view_name}", f"Exception: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
            )
    ...
    def display_financial_impact_assessment(self, cross_comparator):
        ...
        for comp in cross_comparator.item_comparisons:
            if comp.result_type.value == "missing_in_profittabilita" and comp.pre_item:
                ...
                missing_in_prof_value += cross_comparator._safe_float(item.get("pricelist_total_price", 0))
            elif comp.result_type.value == "missing_in_pre" and comp.prof_item:
                item = comp.prof_item["item_data"]
                missing_in_pre_value += cross_comparator._safe_float(item.get("pricelist_total", 0))
            elif comp.result_type.value == "value_mismatch":
                if comp.pre_item and comp.prof_item:
                    pre_value = cross_comparator._safe_float(comp.pre_item["item_data"].get("pricelist_total_price", 0))
                    prof_value = cross_comparator._safe_float(comp.prof_item["item_data"].get("pricelist_total", 0))
        ...
        impact_data = {
            ...
            "Value (€)": [missing_in_prof_value, -missing_in_pre_value, modified_items_impact, diff_amount],
            "Description": [
        ...
        with col1:
            ...
            positive_categories = [
                cat
                for cat, val in zip(df_impact["Category"].tolist()[:-1], df_impact["Value (€)"].tolist()[:-1])
                if val > 0
            ]
        ...
        if abs(diff_percentage) > 10:
            risks.append(
                f"🔴 High financial variance: {diff_percentage:.1f}% difference between PRE and Profittabilita"
            )
        ...
        if cross_comparator.pricelist_analysis["items_with_differences"] > len(cross_comparator.item_comparisons) * 0.1:
            risks.append(
                f"🟡 High number of item discrepancies: {cross_comparator.pricelist_analysis['items_with_differences']} items"
            )
    ...
    def display_project_structure_analysis(self, cross_comparator):
        ...
        with col1:
            # Coverage metric
            items_with_mapping = len(
                [
                    c
                    for c in cross_comparator.item_comparisons
        ...
        with col2:
            ...
            consistency = (
                (perfect_matches / len(cross_comparator.item_comparisons) * 100)
                if cross_comparator.item_comparisons
                else 0
            )
        ...
        with col3:
            ...
            avg_items_per_wbe = (
                (sum(wbe["items_count"] for wbe in cross_comparator.wbe_map.values()) / len(cross_comparator.wbe_map))
                if cross_comparator.wbe_map
                else 0
            )
    ...
    def run(self):
        ...
        st.set_page_config(
            page_title="Project Structure Analyzer", page_icon="🏗️", layout="wide", initial_sidebar_state="expanded"
        )
        ...
        with st.sidebar:
            ...
            try:
                ...
                if operation_mode == OperationMode.ANALYZE_PRE:
                    ...
                    data, file_name, _ = self.render_single_file_upload(
                        FileType.ANALISI_PROFITTABILITA, "💹 Profittabilita Analysis"
                    )
                    if data is not None:
                        self.current_data = data
                    ...
                    analyzer, file_name, detected_type = self.render_unified_file_upload(
                    ...
                    data1, data2, file1_name, file2_name = self.render_dual_file_upload(
                        FileType.PRE_FILE, "🔄 PRE Files Comparison"
                    )
                    if data1 is not None and data2 is not None:
                        ...
                        self.current_comparator = self.create_comparator(
                            data1, data2, FileType.PRE_FILE, self.file1_name, self.file2_name
                        )
                        st.session_state.comparator_initialized = True

                elif operation_mode == OperationMode.COMPARE_PROFITTABILITA:
                    data1, data2, file1_name, file2_name = self.render_dual_file_upload(
                        FileType.ANALISI_PROFITTABILITA, "⚖️ Profittabilita Comparison"
                    )
                    if data1 is not None and data2 is not None:
                        self.data1 = data1
                        self.data2 = data2
                        ...
                        self.file2_name = file2_name or "File 2"
                        self.current_comparator = self.create_comparator(
                            data1, data2, FileType.ANALISI_PROFITTABILITA, self.file1_name, self.file2_name
                        )
                        st.session_state.comparator_initialized = True
                    ...
                    if pre_data is not None and prof_data is not None:
                        ...
                        self.cross_comparator = self.create_cross_comparator(
                            pre_data, prof_data, self.pre_file_name, self.prof_file_name
                        )
            ...
            if self.current_analyzer is not None and operation_mode in [
                OperationMode.ANALYZE_PRE,
                OperationMode.ANALYZE_PROFITTABILITA,
                OperationMode.ANALYZE_UNIFIED,
            ]:
                try:
                    selected_view = render_navigation_sidebar(self.current_analyzer, st.session_state.current_view)
                    ...
                    # PDF Report section
                    render_pdf_export_button(self.current_data, self.current_file_type)

                except Exception as e:
                    render_error_message(
                        "Error in navigation", f"Exception: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
                    )
            ...
            # Navigation for comparison mode
                ...
                try:
                    ...
                    selected_view = st.sidebar.radio(
                        ...
                        views,
                        index=(
                            views.index(st.session_state.comparison_view)
                            if st.session_state.comparison_view in views
                            else 0
                        ),
                    ...
                    render_error_message(
                        "Error in comparison navigation", f"Exception: {str(e)}\n\nTraceback:\n{traceback.format_exc()}"
                    )
            ...
            # Navigation for cross-comparison mode
                ...
                try:
                    ...
                    selected_view = st.sidebar.radio(
                        ...
                        views,
                        index=(
                            views.index(st.session_state.cross_comparison_view)
                            if st.session_state.cross_comparison_view in views
                            else 0
                        ),
        ...
        if operation_mode in [
            ...
            if (
                ...
                and st.session_state.analyzer_initialized
            ):
            ...
            if (
                ...
                and self.data1 is not None
                and self.data2 is not None
                and st.session_state.comparator_initialized
            ):
            ...
            if (
                self.cross_comparator is not None
                and self.pre_data is not None
                and self.prof_data is not None
                and st.session_state.cross_comparator_initialized
            ):
